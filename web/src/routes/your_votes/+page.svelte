...
{:else}
  ...
  <p>
    Here are
    {#if data.latestVotes.length < data.totalVotes}
      the last {data.latestVotes.length} posts
    {:else}
      all the posts
    {/if}
  ...
  <p>
    This page allows you to change or delete your most recent votes. If you want
    to change or delete any votes not listed here, please get in touch with me
    via Discord (<code>@is_a_togekiss</code>) or
  ...
  <table>
    ...
    <tbody>
      {#each data.latestVotes as v}
        <tr>
          <td><code>{v.post.id}</code></td>
          <td><a href={v.post.url}>{v.post.title}</a></td>
          <td>/u/{v.post.submitter}</td>
          <td>{v.vote ? "Yes" : "No"}</td>
          <td
            ><form action="/your_votes?/change" method="POST">
              <input type="hidden" name="id" value={v.post.id} />
